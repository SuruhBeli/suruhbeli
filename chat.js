...
// ===== LOGIN CEK ===== //
// ===== TUNGGU AUTH SIAP DARI index.js =====
window.addEventListener("app-ready", async () => {
  ...
  console.log("✅ Chat siap untuk:", currentUser.uid);

  // Tunggu IndexedDB kalau ada
  if (window.idbReady) {
    await window.idbReady;
  }

  // Bersihkan listener lama (anti dobel)
  if (window.cleanupRoomListeners) {
    window.cleanupRoomListeners();
  }

  // 🔥 INIT SEMUA FITUR CHAT
...
document.getElementById("backBtn").addEventListener("click", () => {
  window.location.href = "index.html#chatlist";
});
